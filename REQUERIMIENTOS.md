...
- **Nombre del equipo:** Atlas
- **Nombre del proyecto:** Atlas
- **Integrantes y roles:**
  | Integrante | Rol | Responsable de |
  |-----------|-----|----------------|
  | (Equipo Atlas) | DevOps | Repo, Git, CI/CD, despliegue |
  | (Equipo Atlas) | Frontend | Interfaz, estilos, interacción |
  | (Equipo Atlas) | Frontend | Interfaz, estilos, interacción |
  | (Equipo Atlas) | Backend | Servidor, API, lógica + BD |
  | (Equipo Atlas) | Backend / PM | Servidor, API, lógica + BD / Canal con el cliente, QA, entregas, desbloquear al equipo |
...
**La idea en una frase:** _Atlas es un sistema de gestión de reciclaje inteligente que usa inteligencia artificial (YOLOv8) para detectar botellas PET, sus tapas y etiquetas mediante la cámara, y recompensa a los usuarios con puntos canjeables (AtlasPuntos)._

- **¿Para quién es? (usuarios):** Comunidad educativa, empresas y público externo interesado en reciclar.
- **¿Qué problema resuelve o qué permite hacer?:** Incentiva el reciclaje de botellas PET eliminando el conteo manual: la IA identifica la botella, la tapa y la etiqueta, asigna puntos automáticamente (50 base + 10 tapa + 5 etiqueta), genera comprobantes de cada reciclaje y permite canjear los AtlasPuntos por productos ecológicos del catálogo.
- **Visión (a dónde quieren llevarlo):** Convertirnos en la plataforma de reciclaje con recompensas de referencia en instituciones de Colombia, expandiendo los materiales detectables (latas, papel, plásticos varios) e integrando estaciones físicas de acopio con reconocimiento en tiempo real.
- **Modelo:** **B2B2C** — se implementa dentro de instituciones (B2B: la institución u otras organizaciones) pero el valor lo vive el usuario final (B2C: quien recicla y canjea).
...
| Sistema de AtlasPuntos (50 base + 10 tapa + 5 etiqueta) con historial de movimientos | ✅ | | Backend |
| Comprobante automático por cada reciclaje (ATLA-000001, …) | ✅ | | Backend |
...
- [x] **Feature clave:** **Detección de botellas PET con IA (YOLOv8)**: el usuario toma una foto con la cámara y el modelo entrenado identifica botella, tapa y etiqueta, calculando los AtlasPuntos ganados automáticamente.
...
usuarios(id_usuario, nombre_completo, tipo_documento, numero_identificacion,
         correo, clave_hash, rol, puntos, fecha_registro)
...
- **Dominio:** subdominio gratis del host (`atlas.onrender.com`).
- **CI/CD:** ¿cada `push` actualiza el sitio? **Sí** — Render redespliega automáticamente con cada push a la rama principal.
- **Link del proyecto (cuando exista):** _https://atlas.onrender.com_ (por definir)
...
| 11–13 (feature / auth / tiempo real) | Feature clave: escaneo con cámara + detección YOLOv8 + asignación de AtlasPuntos y comprobantes |
